...
void Octree::subdivide(const ofMesh & mesh, TreeNode & node, int numLevels, int level) {
    ...
    int nodePoints = node.points.size();
    int total = 0;
    for (int i = 0; i < boxes.size(); i++) {
        TreeNode child;
        int count = 0;
        if (bUseFaces) {
            count = getMeshFacesInBox(mesh, node.points, boxes[i], child.points);
        }
        else {
            count = getMeshPointsInBox(mesh, node.points, boxes[i], child.points);
        }
        total += count;
        if (count > 0) {
            child.box = boxes[i];
